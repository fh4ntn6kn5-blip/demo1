...
const MENTION_ITEMS: MentionItem[] = [
  {
    ...
    avatar: "/images/sugar-avatar.png",
  },
  {
    id: "member1",
    type: "member",
...
export default function MentionPopover({ position, visible, onSelect, highlightedIndex }: MentionPopoverProps) {
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-medium truncate ${isSugar ? "text-indigo-600" : "text-gray-800"}`}>
                  {item.name}
                </div>
                <div className="text-xs text-gray-500 truncate">{item.desc}</div>
              </div>
              {isSugar && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-600 font-medium">
                  AI
                </span>
              )}
            </button>

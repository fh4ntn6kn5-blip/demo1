...
function TreeNode({ node, level = 0, onSelect, selectedName }: TreeNodeProps) {
    ...
    return (
      <button
        ...
        className={`w-full min-w-0 box-border pr-2 flex items-center gap-2 py-1.5 text-left text-sm rounded-lg transition-colors ${
          isSelected ? "bg-indigo-500/10 text-indigo-600" : "text-gray-700 hover:bg-white/70"
        }`}
...
interface SidebarProps {
  onSelectDoc?: (doc: TreeNodeData | null) => void
  selectedName?: string | null
}

export default function Sidebar({ onSelectDoc, selectedName }: SidebarProps) {
  return (
      ...
      <div className="h-full overflow-y-auto py-5 px-4">
        ...
        <div className="mt-6">
          ...
          <div className="space-y-1">
            {treeData.map((node) => (
              <TreeNode key={node.id} node={node} onSelect={onSelectDoc} selectedName={selectedName} />
            ))}
          </div>
        </div>
      </div>

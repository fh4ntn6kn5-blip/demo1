...
import MentionPopover, { type MentionItem } from "./mention-popover"

const sugarAvatar = "/sugar-avatar.png"
...
const contextSummaryFull = "正在为你汇总"知识库团队周报-随心搭"相关数据、项目进展、用户反馈"
...
export default function MainEditor({ docTitle = "未命名文档" }: { docTitle?: string }) {
  ...
  const dataAnchorRef = useRef<HTMLElement | null>(null)

  const mentionItemsCount = 5

  const getCaretRect = useCallback((range: Range) => {
  ...
  const findDataAnchor = useCallback(() => {
    ...
    if (anchor) dataAnchorRef.current = anchor as HTMLElement
  ...
  const removeAtTokenAtCaret = useCallback(() => {
    const editor = editorRef.current
  ...
  const selectMentionByIndex = useCallback(
    ...
    [mentionPosition, removeAtTokenAtCaret]
  )
  ...
  const ensureStatusLine = useCallback((text: string, attempt = 0) => {
    const editor = editorRef.current
    if (!editor) {
      if (attempt < 10) {
  ...
  const insertStatusLineAtCaret = useCallback(() => {
    const editor = editorRef.current
  ...
  const insertDashboardInline = useCallback(
    (queryText: string, attempt = 0) => {
      const editor = editorRef.current
      if (!editor) {
        if (attempt < 10) {
      ...
      const setTab = (name: string) => {
        tabContents.forEach((content) => {
          ;(content as HTMLElement).style.display =
            content.getAttribute("data-tab-content") === name ? "block" : "none"
        })
  ...
  }, [ensureStatusLine, insertStatusLineAtCaret, clearStatusLine, insertDashboardInline])
    ...
    <main className="flex-1 flex flex-col min-w-0 bg-transparent">
          ...
          <div className="relative h-full min-h-[calc(100vh-210px)] bg-white/65 backdrop-blur-2xl rounded-3xl shadow-[0_24px_60px_-40px_rgba(17,24,39,0.32)] border border-white/50 px-10 pt-10 pb-0 flex flex-col overflow-hidden">
            <div ref={editorScrollRef} className="flex-1 overflow-y-auto pr-0 hide-scrollbar">
              <div className="relative min-h-[360px]">
                ...
                {lastInsertHint && (
                  <div className="absolute right-0 top-0 text-xs text-indigo-500 bg-indigo-500/10 px-2 py-1 rounded-md">
                    {lastInsertHint}
                  </div>
                )}
                  ...
                  {sugarMode && (
                    ...
                    >
                      <div className="relative rounded-xl overflow-hidden">
                        ...
                        <div className="relative bg-white rounded-xl border border-indigo-500/30 shadow-[0_0_0_1px_rgba(99,102,241,0.3),0_0_20px_-5px_rgba(99,102,241,0.4)] p-1">
                          <div className="flex items-center gap-3 p-3">
                            <div className="w-9 h-9 rounded-xl overflow-hidden ring-1 ring-white/70 shadow-[0_8px_18px_-14px_rgba(0,0,0,0.3)] flex-shrink-0 relative">
                              <Image
                                src={sugarAvatar}
                                alt="Sugar 智能体"
                                fill
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="text-[13px] font-semibold text-indigo-500 tracking-wide">
                                Sugar 智能体
                              </div>
                              <input
                                ref={inputRef}
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-500 text-white text-sm font-medium hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                              >
      ...
      <AnimatePresence>
        {showContextHint && !contextPanelOpen && (
          ...
          >
            <button onClick={() => setContextPanelOpen(true)} className="hint-glow">
              <div className="hint-glow-inner flex items-center gap-3 px-4 py-2 rounded-full bg-white/95 backdrop-blur border border-white/90 shadow-[0_12px_30px_-18px_rgba(17,24,39,0.5)] text-sm text-gray-700">
                <div className="w-6 h-6 rounded-full overflow-hidden ring-1 ring-white/70 relative">
                  <Image src={sugarAvatar} alt="Sugar 智能体" fill className="object-cover" />
                </div>
                <span className="text-indigo-500 font-semibold">Sugar 智能体</span>
                <span>正在为你检测随心搭数据</span>
                <span className="text-indigo-500 font-semibold">查看</span>
              </div>
            </button>
          </motion.div>
        )}
                  ...
                  <div className="rounded-2xl border border-white/60 bg-white/80 p-4 shadow-[0_14px_30px_-22px_rgba(15,23,42,0.4)]">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs rounded-full bg-indigo-500 text-white hover:bg-indigo-600 transition-colors"
                      >
                        插入
                      </button>
                      <button

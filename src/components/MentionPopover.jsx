...
const avatarImg = new URL('../../img/infoflow 2026-01-31 17-57-18.png', import.meta.url).href
...
export default function MentionPopover({ position, visible, onSelect, highlightedIndex }) {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                isHighlighted ? 'bg-accent-purple/10' : 'hover:bg-white/70'
              } ${isSugar && isHighlighted ? 'ring-1 ring-inset ring-accent-purple/20' : ''}`}
            >
              {item.avatar ? (
                <div className="w-9 h-9 rounded-xl overflow-hidden ring-1 ring-white/60 shadow-[0_8px_20px_-14px_rgba(0,0,0,0.3)] flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <div
                  className={`text-sm font-medium truncate ${isSugar ? 'text-accent-purple' : 'text-gray-800'}`}
                >
                  {item.name}
                </div>
